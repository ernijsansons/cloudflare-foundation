...
<svelte:head>
	<title>{data.agent?.name || 'Agent'} - Naomi | ERLVINC Dashboard</title>
</svelte:head>
...
<div class="min-h-screen bg-gray-900 text-white p-8">
	...
	<nav class="mb-6">
		<ol class="flex items-center space-x-2 text-sm text-gray-400">
			<li><a href="/agents" class="hover:text-purple-400">Agents</a></li>
			<li><span class="mx-2">/</span></li>
			<li><span class="text-purple-400">Naomi</span></li>
			<li><span class="mx-2">/</span></li>
	...
	{:else if data.agent}
		<!-- Agent Header -->
		<div class="bg-gray-800 rounded-xl p-6 mb-6 border-l-4 border-purple-500">
			<div class="flex items-start justify-between">
				<div>
					...
					<div class="flex items-center gap-4 text-sm text-gray-400">
						<span class="bg-purple-500/20 text-purple-300 px-2 py-1 rounded">
							{roleLabels[data.agent.role]}
						</span>
						<span>ID: {data.agent.id}</span>
						<span class="text-purple-400">Source: Naomi</span>
					</div>
				</div>
		...
		{#if data.agent.capabilities && data.agent.capabilities.length > 0}
			<div class="bg-gray-800 rounded-xl p-6 mb-6">
				<h2 class="text-lg font-semibold mb-4">Scopes & Capabilities</h2>
		...
		{#if data.raw}
			<div class="bg-gray-800 rounded-xl p-6">
						...
						<div class="space-y-2">
							{#each data.raw.tasks.slice(0, 5) as task}
								<div class="bg-gray-700/50 rounded-lg p-3 flex items-center justify-between">
									<div>
										<div class="text-sm font-medium">{task.task_id}</div>
										{#if task.description}
				...
				<!-- Capability Tokens -->
				{#if data.raw.capabilities && data.raw.capabilities.length > 0}
					<div>
						<h3 class="text-md font-medium text-gray-300 mb-3">Capability Tokens</h3>
						<div class="space-y-2">
							{#each data.raw.capabilities as cap}
								<div class="bg-gray-700/50 rounded-lg p-3">
									<div class="flex items-center justify-between">
										<code class="text-xs text-purple-300">{cap.cap_id}</code>
										{#if cap.expires_at}
